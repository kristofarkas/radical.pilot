    ...
    def _start_sub_agents(self):
        ...
        for sa in self._cfg['agents']:
            ...
            if target == 'local':
                ...
                lrms_info = self._cfg['lrms_info']
                pwd       = os.getcwd()
                ls_name   = '%s/%s.sh' % (pwd, sa)
                slots     = {'cpu_processes' : 1,
                             'cpu_threads'   : 1,
                             'gpu_processes' : 0,
                             'gpu_threads'   : 0,
                             'nodes'         : [[node[0], node[1], [[0]], []]],
                             'cores_per_node': lrms_info['cores_per_node'],
                             'gpus_per_node' : lrms_info['gpus_per_node'],
                             'lm_info'       : lrms_info['lm_info']
                             }
                agent_cmd  = {'uid'          : sa,
                              'slots'        : slots,
                              'description'  : {
                                  ...
                                  'executable'   : "/bin/sh",
                                  'mpi'          : False,
                                  'arguments'    : ["%s/bootstrap_2.sh" % pwd, sa]
                              }
                             }
                ls_hop   = '/usr/bin/env RP_SPAWNER_HOP=TRUE "%s"' % ls_name
                cmd, hop = agent_lm.construct_command(agent_cmd,
                                                      launch_script_hop=ls_hop)
